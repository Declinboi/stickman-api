...
class EffectsRenderer:
    ...
    def __init__(self) -> None:
        ...
        # Track previous wrist positions for velocity-based punch detection
    ...
    def trigger_punch(self, x: int, y: int) -> None:
        """Spawn a punch flash effect at (x, y)."""
        self.active_effects.append(ActiveEffect(
            kind="punch",
            x=x, y=y,
            lifetime=8, max_lifetime=8,
            color=(255, 220, 0),   # yellow flash
    ...
    def trigger_sword_trail(self, x1: int, y1: int, x2: int, y2: int) -> None:
        ...
        self.active_effects.append(ActiveEffect(
            kind="sword_trail",
            x=x1, y=y1,
            lifetime=6, max_lifetime=6,
            color=(180, 230, 255),  # icy blue-white
            size=3,
            points=[(x1, y1), (x2, y2)],
        ))
    ...
    def trigger_fall_dust(self, x: int, y: int) -> None:
        ...
        for _ in range(12):
            angle = random.uniform(np.pi, 2 * np.pi)  # upward arc
    ...
    def detect_and_trigger_punch(
        self, landmarks: dict, prev_landmarks: Optional[dict]
    ) -> None:
        """
        Automatically detect punch impacts from wrist velocity.
        If a wrist moves more than 25px between frames, trigger a punch flash
        near the moving wrist.
        """
        if not prev_landmarks:
            return

        for wrist_idx in [15, 16]:  # LEFT_WRIST, RIGHT_WRIST
            ...
            if velocity > 25:
                # Punch impact point — slightly ahead of the wrist
    ...
    def detect_and_trigger_fall(
        ...
        for hip_idx in [23, 24]:  # LEFT_HIP, RIGHT_HIP
    ...
    def detect_and_trigger_sword(
        self, landmarks: dict, has_sword: bool
    ) -> None:
        """
    ...
    def render(self, frame: np.ndarray) -> np.ndarray:
        ...
        for effect in self.active_effects:
            alpha = effect.lifetime / effect.max_lifetime  # fades out over time
    ...
    def _draw_punch(
        self, frame: np.ndarray, effect: ActiveEffect, alpha: float
    ) -> None:
        """Star-burst / comic punch flash."""
    ...
    def _draw_sword_trail(
        ...
        """Glowing sword slash trail between two points."""
        ...
        # Draw three overlapping lines for a glow effect
    ...
    def _draw_blood(
        ...
        # Interpolate particle position based on remaining lifetime
        ...
        color = self._scale_color(effect.color, alpha)
        cv2.circle(frame, (px, py), effect.size, color, -1, cv2.LINE_AA)

    def _draw_dust(
        self, frame: np.ndarray, effect: ActiveEffect, alpha: float
    ) -> None:
        """Fall dust particle — drifts upward and fades."""
        if len(effect.points) < 2:
            return
    ...
    def _scale_color(
        ...
        """Scale a BGR color by alpha for fade-out effect."""
